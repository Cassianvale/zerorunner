...
class Configs(BaseSettings):
    ...
    # redis
    REDIS_URI: str = Field(default="redis://:@localhost:6379/4", env="REDIS_URI")

    # DATABASE_URI: str = "sqlite+aiosqlite:///./sql_app.db?check_same_thread=False"  # Sqlite(异步)
    DATABASE_URI: str = Field(
        default="mysql+aiomysql://root:123456@localhost:3306/zerorunner?charset=UTF8MB4", env="MYSQL_DATABASE_URI"
    )  # MySQL(异步)
    ...
    # celery worker
    broker_url: str = Field(default="redis://:@localhost:6379/5", env="CELERY_BROKER_URL")
    ...
    # job beat
    beat_db_uri: str = Field(default="mysql+pymysql://root:123456@localhost:3306/zerorunner", env="CELERY_BEAT_DB_URL")
    ...
    GITLAB_USER: str = Field(None, env="GITLAB_USER")
